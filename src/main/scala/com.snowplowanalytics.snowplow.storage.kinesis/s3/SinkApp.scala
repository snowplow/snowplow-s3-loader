...
object SinkApp extends App {
  ...
  val kinesisSinkEndpoint = kinesisSinkRegion match {
      case "cn-north-1" => "kinesis.cn-north-1.amazonaws.com.cn" // adding AWS China kinesis endpoint
      case _ => s"https://kinesis.${kinesisSinkRegion}.amazonaws.com"
  }
  ...
  def convertConfig(conf: Config, credentials: AWSCredentialsProvider): KinesisConnectorConfiguration = {
    ...
    val kEndpoint = kinesisRegion match {
