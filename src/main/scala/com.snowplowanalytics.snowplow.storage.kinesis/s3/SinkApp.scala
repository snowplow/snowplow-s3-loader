...
object SinkApp {
  ...
  def main(config: S3LoaderConfig): Unit = {
    val tracker = config.monitoring.map(SnowplowTracking.initializeTracker(_))
    ...
    val kinesisSinkName = config.streams.streamNameOut
    ...
    */
    def convertConfig(conf: S3LoaderConfig, credentials: AWSCredentialsProvider): KinesisConnectorConfiguration = {
      ...
      props.setProperty(KinesisConnectorConfiguration.PROP_KINESIS_INPUT_STREAM, conf.streams.streamNameIn)
