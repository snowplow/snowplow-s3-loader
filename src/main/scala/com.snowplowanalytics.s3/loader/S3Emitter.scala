  ...
  def attemptEmit(namedStream: NamedStream, partition: Boolean, connectionAttemptStartTime: Long): Boolean = {
    ...
    while (true) {
      ...
      try {
        ...
        val s3Key = DynamicPath.decorateDirectoryWithTime(config.outputDirectory, namedStream.filename, connectionAttemptStartDateTime, config.dateFormat, config.filenamePrefix)
