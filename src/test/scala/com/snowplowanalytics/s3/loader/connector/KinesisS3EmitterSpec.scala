...
import com.snowplowanalytics.s3.loader.Config.{Format, S3Output}
...
class KinesisS3EmitterSpec extends Specification {
  "KinesisS3Emitter" should {
    ...
    "format file name with optional components" in {
      val s3Config = S3Output(s"s3://no-bucket/$outputDirectory", Some(dateFormat), Some(filenamePrefix), Format.Gzip, 0, None)
    ...
    "format file name without optional components" in {
      val s3Config = S3Output("s3://no-bucket", None, None, Format.Gzip, 0, None)
    ...
    "format file name with path, but without optional components" in {
      val s3Config = S3Output(s"s3://no-bucket/$outputDirectory", None, None, Format.Gzip, 0, None)
      val actual = KinesisS3Emitter.getBaseFilename(s3Config, firstSeq, lastSeq)(None)
    ...
    "format file name with path and partition" in {
      ...
      actual.replaceAll("\\d{4}-\\d{2}-\\d{2}-\\d{6}", "2021-04-30-000000") must beEqualTo(s"$outputDirectory/partition/2021-04-30-000000-$firstSeq-$lastSeq")
    }
  }
}
